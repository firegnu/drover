...
finish() {
  ...
  clear_wake_for "${OUT}"   # 领到了这个哨兵的结果，等它的标记作废；那个进程下一轮自己退出
...
clear_wake_for() {       # $1=哨兵文件
  [ -f "${WAKE_MARK}" ] && [ "$(sed -n '1p' "${WAKE_MARK}")" = "$1" ] && rm -f "${WAKE_MARK}"
  return 0
}
...
wait_sentinel() {        # $1=file  $2=word  $3=pane_id
  ...
  sentinel_ok "$1" "$2" && { clear_wake_for "$1"; return 0; }
...
.mode,.kind,.flow,.badge,.lvl,.sev,.verb,.dec,.next,.hand,.hold-tag,.chip,.evbtn,.me,.accum{display:inline-block;font-size:var(--fs1);font-weight:600;line-height:18px;padding:0 7px;border-radius:var(--r1);border:0;white-space:nowrap}
...
.accum{font-size:var(--fs2);font-weight:400;margin:20px 8px 0 0;padding:0 10px;line-height:26px;background:var(--s1);border:1px solid var(--line);color:var(--t3)}
details.mapsug{margin-top:8px;font-size:var(--fs2)}details.mapsug>summary{color:var(--amber-t);cursor:pointer}details.mapsug li{margin:3px 0 3px 16px}.accum.warn{color:var(--amber-t);border-color:#5e4c2c;background:#2d2a25}.accum b{font-weight:700}
...
.srow{display:grid;grid-template-columns:92px 72px 120px minmax(0,1fr);gap:0 12px;padding:8px 0;border-bottom:1px solid var(--line-soft)}
...
ol.q li,.drow,.card,.bhead,.crew .agent,.accum,.proj.sel{box-shadow:inset 0 1px 0 rgba(255,255,255,.05)}
...
def render_tasks(p, live=None):
    ...
    if tv["loop"] and lw:
        ...
        doing += (f'<div class="loopw bad">自动叫醒失败：{esc(lw.get("error", ""))}。对写手说「运行 review-task next，按它的输出办」</div>' if lw.get("failed")
                  else f'<div class="loopw">写手停着（{esc(why)}）· 条件满足后自动叫醒</div>')
    fin = []
...
def crew_chips(p):
    """写手 / 规划者 / 评审方的状态块（herdr 里的状态、标题、正在做什么）；静态页、本机服务页和 /crew 共用。"""
    ag = p.get("agents") or {}
    chips = []
    for role, label in (("writer", "写手"), ("planner", "规划者"), ("reviewer", "评审方")):
        a = ag.get(role)
...
def render_panel(p, archives, self_closed, live=None):
    ...
    # 有评审轮次，或 request.md 真写了 target：才算有在途周期。等你放行、规划中这类状态本身不是评审周期，不画空卡片
    elif p["rounds"] or (p["req"].get("target sha") and p["state"] not in ("空闲",) and not p["state"].startswith("triage")):
        parts.append(render_cycle(p))
        for rnd in p["rounds"]:
            parts.append(render_round(p, rnd))
...
def collect(list_path):
    ...
    global NOW, _AGENTS
    NOW, _AGENTS = time.time(), None                 # herdr 的 agent 列表同理：每页重查
...
LOOP_PROMPT = "运行 review-task next，按它的输出办"


def herdr_agent(pane):
    try:
        out = subprocess.run([HERDR, "agent", "get", pane], capture_output=True, text=True, timeout=5).stdout
        return json.loads(out).get("result", {}).get("agent") or None
    except Exception:
        return None


def loop_tick(list_path):
    """外层循环的叫醒：循环开着、写手停在队列那一步留了 .loop-wait 的项目，等「next 能发出任务」（有待办、没暂停、不等放行）
    且写手空闲时，往它的 pane 输入那句开工的话。第一次看到标记先向 herdr 核对 pane 确实是这个仓库的写手、记下 terminal / session；
    之后身份变了就不叫，标成失败留给人。叫醒失败重试 3 次。"""
    for repo in discover(list_path):
        d = parse_conf(f"{repo}/.review.conf").get("REVIEW_DIR")
        ...
        try:
            ...
            m = None
        if not isinstance(m, dict) or m.get("failed") or not m.get("pane"):
            continue
        def save(**kw):
            m.update(kw)
            ...
            with open(f"{d}/.loop.log", "a", encoding="utf-8") as f:
                f.write(f"{datetime.now():%Y-%m-%d %H:%M:%S} {m['pane']} {json.dumps(kw, ensure_ascii=False)}\n")
        a = herdr_agent(m["pane"])
        if not a:
            save(attempts=m.get("attempts", 0) + 1, error="herdr 里查不到这个 pane")
            if m["attempts"] >= 3:
                save(failed=True)
            continue
        session = (a.get("agent_session") or {}).get("value") or ""
        if "terminal" not in m:
            if os.path.realpath(a.get("cwd") or "") != os.path.realpath(repo):
                save(failed=True, error=f"pane {m['pane']} 不是这个仓库的写手（cwd {a.get('cwd')}）")
                continue
            save(terminal=a.get("terminal_id") or "", session=session)
        if a.get("terminal_id") != m["terminal"] or (m.get("session") and session and session != m["session"]):
            save(failed=True, error=f"写手 pane {m['pane']} 的身份变了（换了 terminal 或 session），没有叫")
            continue
        tasks, current, awaiting = task_fold(task_events(read(f"{d}/tasks.state")))
        ...
        if r.returncode == 0 or after in ("working", "blocked"):
            ...
            save(attempts=m.get("attempts", 0) + 1, error=(r.stderr or r.stdout or "注入失败").strip()[:200])
            if m["attempts"] >= 3:
                save(failed=True)
...
def serve(args):
    ...
    class Handler(http.server.BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if path == "/crew":
                return self.reply(200, {x["name"]: crew_chips(x) for x in collect(list_path)[0]})
            if path == "/history":
...
**循环怎么转。** 你启动写手，对它说「运行 review-task next，按它的输出办」。`AGENTS.md` 没改，写手只能从这句话知道有队列；之后每一步该做什么都写在命令的输出里。写手运行 `review-task next` 拿到任务，末尾写着做完后运行 `review-task done T5`；它按 AGENTS.md 的流程做完，再运行 `done`。`done` 由工具只读核对，不听写手一句话：
...
**命令**（在仓库目录里运行）：`add "标题" [说明行 …]` 加到队列末尾并自动编号；`list` 看全貌；`go` 放行；`drop T5 "原因"` 放弃（`drop --pos 3 "原因"` 按队列位置放弃还没开始的任务，没编号的也行：编号记进 `tasks.state`、按标题认，之后再手写同名的没编号任务也会被当成已放弃，换个标题或加上编号）；`pause` / `resume` 暂停或恢复发新任务，正在做的照常做完；`move 4 1` 把队列里第 4 个还没开始的任务挪到第 1 个，`edit 2 "标题" [说明行 …]` 整块改写第 2 个（编号不变）——位置就是 `list` 里「队列」的序号，没编号的任务也能这样认；`move` / `edit` / `drop --pos` 都可带 `--expect <指纹>`（`queue.md` 内容的短 sha1），对不上说明这期间队列被别人改过，什么都不写、退出码 10。改写时先写临时文件再整体替换，前言和开始过的块原地不动。退出码：0 发出任务或操作成功；8 停下把输出报告给人（队列空、暂停、等放行）；9 还没收尾；2 用法错误；10 队列在这期间被改过（`--expect` 对不上）。
