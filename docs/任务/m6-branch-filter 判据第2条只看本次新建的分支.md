...
2026-09-21，drover/main。**通过，已合并进 main（本地，未推送）。**

两轮：第一轮实现 + 第一轮交叉审查提出 2 必须改 2 建议改 → 返工 → 复核「可以合并」。

主控自己核过的（没有只信完成记录）：

- 四个套件自己重跑，合并前后都全绿。
- **另造合成仓库独立验 RED→GREEN**，不用它的测试：旧实现 `ok=False 还没合进 main：m4/planning`（死锁复现），修复版四种情形全对（遗留分支在且本次已合入→过并点名排除项；全是遗留→不过；glob 没匹配→不过且说法不同；`base_sha` 空→不适用）。
- **独立复现交叉审查那个对象损坏实验**：第一轮版 `c765318` 确实 `ok=True` 误过，还把损坏的未合入分支写成「遗留分支」——审查意见属实，不是误报。
- **自己跑两个变异**确认新测试挡得住回归：`current[:1]` 被 `second current branch must still block` 抓住；退回 `returncode == 0` 被 `branch query error must block despite merged siblings` 抓住。
- 范围：只动该动的四个文件，`criteria` 签名没动，没加配置项，没碰 `bin/drover` / `find_done_mark` / `HANDOFF.md`，没合并没推送。「不要做」一条没犯。

取舍逐条表态：

1. `milestone_branches` 返回多元组——**同意**。两个直接调用点都适配了，`criteria` 接口不变。返工后扩成三元组（加 errors），同理。
2. 文案公开判别依据、不引入日期或 reflog 猜测——**同意**。
3. 暂留 ROADMAP「修法（还没做）」不越权改——**同意且赞赏边界感**。落地状态由主控在合并收尾时统一更新，已做。

意见 1（判别式证明不了「这次新建」）：**已请人裁定，接受这个边界，不改判别式。** 理由和彻底修的路子都写进 `docs/ROADMAP.md` 该节「这条判据的已知限制」了。
