...
2026-09-20，drover/main 审完，**可以合并**（合并提交见 `git log --merges`）。三轮。

**第一轮**：范围干净（只动 `bin/drover-board`、`bin/drover` 的通知那点、`tests/drover-board.sh`、删 `browser-smoke.mjs`）；21 个被保护的数据模块函数签名逐个核过，一个没丢；`tests/drover.sh` 和 `tests/criteria.sh` 确实一行没动。我把 main（已含 m5-install）并进它的分支跑了四个套件——全绿、无冲突，这步它自己没做，我替它做了。另外用 pty 独立开了一次 TUI：退出码 0、不崩、没碰真实家目录。
→ 交回一件**必须改**：按键是新的主操作面（ROADMAP：「推进靠按键，不靠敲命令」），而「按了什么键 → 做什么」还埋在 `tui()` 里跟 `stdscr.getch()` 缠着，一条自动测试都没有；`p` 和 `l` 还是有逻辑的（按当前状态决定发 `pause` 还是 `resume`、`loop on` 还是 `off`），切反了就是真 bug。

**第二轮**：`key_action(k, pv, state)` 拆成纯函数，测试先红后绿。我独立验过：`p`（已暂停）→ `resume`、`l`（循环关）→ `loop on`、`j`（sel=2,n=5）→ 3 不越界、生键 → `None`、没有选中项目 → `None`、**调用后 `state` 一个字段都没变**。pty 实测按 `l` 真落出 `loop` 文件、按 `p` 落出 `paused`、`q` 自己退出。
→ 交回一件**建议改但我要求改**：`tick()` 每跳都跑 `B.collect()`，而引擎默认 5 秒一跳。`collect()` 每个项目十来个 git 子进程加一次 corral status，常驻一天一万七千多趟——相对老看板 30 分钟一次静态生成是实打实的退化，而守护进程恰恰要一直开着。

**第三轮**：`NOTIFY_EVERY = 60` 节流，记在内存里（常驻进程，不落文件，不新建配置项）。边界划对了：节流的是「多久看一次」，`board-notified.json` 的去重一个字没动，`loop_tick` 仍旧每跳都跑，两条都单独有断言。我走**真路径**独立验过：`--interval 1` 跑 9 秒，假 corral 只被叫 **1 次**（不节流会是 9 次），真实系统一条通知都没弹。

**取舍逐条表态**：通知节流 60 秒 —— 同意；健康提示只剩 corral 一项 —— 同意（另外四项都是围着已删的 HTTP 服务转的）；`wait_items()` 三元组减成二元组 —— 同意（锚点是网页概念）；`a` 键直接开 `$EDITOR` 编 `queue.md` —— 同意，任务书就是这么要求的；窄屏退化成只画详情 —— 同意；**详情区不滚动、超出截断并提示还有几行** —— 暂时同意，队列长了会不够用，记为后续；消息栏每次刷新还原成按键提示（命令回话最多留到下一次自动刷新）—— 同意，能接受。

**它指出我任务书写错的地方，它是对的**：`width("把 CSS 导入改成流式")` 我写 16、ROADMAP 写「16 格不是 11 格」，实际是 12 个字符、19 格。ROADMAP 已改（`7c87c99`）。

**它主动报的疏忽**：头一版测试三处没盖住 `HOME` 和 `DROVER_NOTIFY_BIN`，写到了真实家目录、真弹了几条 macOS 通知。已修并加了「跑完真 `~/.drover` 的 mtime 不变」的核对。我核过残留：`~/.drover/` 里只有一个 `board-notified.json`，内容是合成仓库（`iota/main`）的条目；`~/.review/` 没被碰。按「删东西先问人」留着没删。

**审查中我自己踩的坑，记下来免得再犯**：头一版 pty 测试没有边读边排空 pty，缓冲填满后子进程写阻塞、整个挂死，跑出来的「按键没生效」是假结果。按 PID 精确停掉那个进程（没用 `pkill -f`，那会把 agent 一起杀），加排空线程后重测才是真结果。**在 pty 里跑 curses 程序，必须一直读。**
