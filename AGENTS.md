...
- 脚本已经改名（第 5 步）：只剩两个可执行文件，`bin/drover`（队列 + 推进 + 判据 + `init` + 引擎 `loop`）和 `bin/drover-board`（面板）。`drover board …` 转给后者。
- **外层循环的引擎是 `drover loop`，独立进程。** 看板是纯观察者，开不开都不影响循环转不转——2026-09-20 从 `board serve` 的线程里拆出来的，别再往回塞。
...
- 审查：主控审查每个任务。
- 合并：审查通过后本地合并进 main。**不推送**——这个仓库还没有远端，要建远端先问人（见下面「先问人」那一节）。
