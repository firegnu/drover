...
不操作真实done/go/next/loop，不推送，T4暂缓。T8兼容实验stash原样保留不恢复/删除。T9已由用户放行，旧记录中的待办描述以本节为准。
...
14. **`drover list` 仍会被标题里的孤立代理码打崩**（`bin/drover:591`）。合成仓库实测：标题带代理码、正文干净时 `drover done` **不崩**（`m14` 的目标达成，退出码 8、报告完整），但 `drover list` 退出 1。**既有问题，`m14` 没制造也没扩大**——任务文件里明写了「只处理会吃到任务正文 / 判据理由的那些，其余不碰」。改法和 `m14` 一样：那几处 `print` 换成 `print_text`。
