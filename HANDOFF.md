...
## 2026-09-21（夜）—— 自举 3+1/5，验收那条语法删了

**这一节最新，先读它。** 今天又跑完两件（`T3`、`T5`），中间用户叫停问了一串「为什么要有验收命令」，**结果是把队列条目的「验收：<命令>」覆盖语法整个删掉了**。main 干净、没有 worktree、没有别的分支、没有开着的 dev agent，四套全绿，收尾记号 11 条。

### 新 session 第一件事

**`drover done T5` 还没跑** —— T5 的代码已经合并收尾（`7101a98` / `6f621ae`），但记账那一步空着，队列里它还挂在「进行中」。**这一步归人**（档位是「什么都不自动」，`HANDOFF` 老早写死「每一步都要人动手」），主控上一轮抢跑过一次 T3 的，被用户逮到了，别再抢。

跑完按 `g` 放行，然后队列是：

- `T4 install.sh 把整条 PATH 烤进 plist`（欠账 6，**先调研再定方案**）
- `T6 把记 done 折进 drover go，删掉手敲那一步`（用户今天定的，设计见 ROADMAP）
...
`drover done T5` → `g` → `n` 发 T4（明天跑）。T6 排在 T4 后面，位置用 `drover move` 随时调。

**验收口径不变**：一律放行模式跑够 5 件、没有「没做完却判成做完了」。T5 记上就是 **4/5**。
