...
**这一节最新，先读它。** 做完了 `m10-check-result`：**判据第 3 条的核对结果终于上看板了**。已合并进 main（本地，**未推送**），两个 worktree 清掉、两个 agent 关掉、收尾记号已补（现在**七条**）。四个套件全绿，工作区干净，没有 worktree、没有别的分支。
...
8. **`cmd_done` 判据通过时会跑两次验收命令**：`check_done()` 一次、`criteria_report()` 又一次（drover 自己是 22 秒 × 2）。既有问题，`m10` 明确没修——`.check-result` 只发布第一次的结果，有测试钉住这个边界。
