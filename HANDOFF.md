...
### corral-dispatch 新流程实测结果（5 处改动全部走到）

都好用，**没有需要 drover 这边改的地方**。三条使用者视角的事实记在这里，**判断归人和 corral 主控做，drover 主控不要插手上游设计**（「只能往下依赖」）：

1. **`--unique` 第一次开就补 `-1` 后缀。** 第 1 节残骸核对那段写着「`--unique` 补出来的 `-2`、`-3` 多半也是（残骸）」——实测第一次开就是 `drover/dev-detail-scroll-1`，所以**光看后缀数字判断不了残骸**。那一节给的命令本身是对的（判据是 `cwd` 还在不在），只是旁边那句话容易让人误判。
2. **复核时，审查者的 detached worktree 不会自己跟上新提交。** 第 6 节说「改完请同一个审查者复核，只看改动部分」，但审查 worktree 还停在被审的旧提交上。这次是我（主控）手动 `git -C <审查 worktree> checkout <新 sha>`（仍是 detached，没切分支）把它推过去的。技能里没写这一步归谁做。
3. **审查文件放在主仓库工作区，detached worktree 里看不到。** 第 6 节没写这个细节，m6 那次是在任务书里自己交代的（给绝对路径 + 「只写这一个文件」），这次照抄，好用。
4. **关掉 agent 之后，之前挂在它身上的 `--after` 提醒照样会投递。** 第 7 节第 5 步关了 agent，但 `corral send --after <它>` 挂的提醒还在队列里，事后仍会送进主控的输入框。这次无害（一查 `corral status` 就是 `not_found` / 退出码 2，认得出是旧的），但**照着提醒直接 `corral reply` 会拿到 not_found**。收到提醒先查状态这条纪律，corral 技能本来就写了，关 agent 之后更要守。

第 7 节那五步按顺序走完全无摩擦：两个 agent 都 `idle` / `attached=0` → 两个 worktree 都干净 → 分支是 main 的祖先 → `worktree remove` 不加 `--force` 两个都删掉、`branch -d` 删掉 → 关掉两个 agent。**「第 5 步的扳机是第 4 步成功返回」这条写得很实用**，不用再纠结「活到底干完没有」。

「关之前确认记录都落盘」这条也确实有用：主控审查、交叉审查意见、复核、实现时的取舍四样，关人之前逐个 `grep` 过在不在 main 上。
